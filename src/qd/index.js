  ...
  if (game._ai.bot && (game.game.turnCounter & 1) === game._ai.bot - 1) {
    suggest(game.game, game._ai.difficulty).then(bestMove => {
      if (bestMove & (0b11 << 8)) {
        ...
        channel.send(`~/qd move W${col}${row}${orientation}`)
      } else {
        const row = 9 - (bestMove & 0b00001111)
        const col = String.fromCharCode(97 + ((bestMove & 0b11110000) >> 4))
        channel.send(`~/qd move ${col}${row}`)
      }
    })
  }
}
...
module.exports = function go (client) {
  client.on('message', message => {
    if (message.content.startsWith('~/qd')) {
      const [ cmd, ...args ] = message.content.substr('~/qd'.length).trim().split(/\s+/)
      switch (cmd) {
        case 'start':
          ...
          if (!p1 || !p2) {
            ...
            message.channel.send('Confirm the match with `~/qd join`.')
